...
public class HSKLine: UIView, HSDrawLayerProtocol {
    ...
    fileprivate func convertToPositionModel(data: [HSKLineModel]) {
        ...
        if maxDiff > 0, maxVolume > 0 {
            ...
            volumeUnit = self.maxVolume == CGFloat.leastNormalMagnitude ? CGFloat.greatestFiniteMagnitude : (lowerChartHeight - theme.volumeGap) / self.maxVolume
        }
            ...
            if let date = klineModels[index].date.hschart.toDate("yyyyMMddHHmmss") {
                ...
                if position.isDrawAxis {
                    ...
                    case .kLineForDay, .kLineForWeek, .kLineForMonth:
                        xAxisTimeMarkLayer.addSublayer(drawXaxisTimeMark(xPosition: position.highPoint.x, dateString: date.hschart.toString("yyyy-MM-dd hh:mm:ss")))
                    default:
                        xAxisTimeMarkLayer.addSublayer(drawXaxisTimeMark(xPosition: position.highPoint.x, dateString: date.hschart.toString("yyyy-MM-dd hh:mm:ss")))
                    }
                    lastDate = date
